...
import { acceptJockeyInvitation, createJockeyProfile, deactivateJockeyProfile, getJockeyInvitations, getJockeyProfile, rejectJockeyInvitation, updateJockeyProfile } from '../../services/jockeyService';
...
export default function JockeyDashboard({ currentUser, onLogout }) {
    ...
    function renderProfileForm() {
        return (<form className="owner-panel owner-form" onSubmit={handleProfileSubmit} noValidate>
        ...
        {!profile && !isLoadingProfile && (<div className="admin-alert error soft-alert" role="alert">Bạn chưa có profile jockey. Hãy tạo profile trước khi nhận lời mời.</div>)}

        {profile && !isProfileActive && (<div className="admin-alert error soft-alert" role="alert">Profile hiện không ACTIVE nên nút accept invitation sẽ bị khóa.</div>)}
        ...
        <input className={profileErrors.licenseNo ? 'input has-error' : 'input'} id="jockeyLicenseNo" name="licenseNo" type="text" placeholder="Ví dụ: JOC-001" value={profileForm.licenseNo} onChange={handleProfileChange} disabled={isSavingProfile}/>
        ...
        <div className="owner-form-row">
          <div>
            ...
            <input className={profileErrors.weight ? 'input has-error' : 'input'} id="jockeyWeight" name="weight" type="number" min="35" max="90" step="0.1" placeholder="55.5" value={profileForm.weight} onChange={handleProfileChange} disabled={isSavingProfile}/>
          ...
          <div>
            ...
            <select className={profileErrors.ranking ? 'input has-error' : 'input'} id="jockeyRanking" name="ranking" value={profileForm.ranking} onChange={handleProfileChange} disabled={isSavingProfile}>
        ...
        <input className={profileErrors.imgUrl ? 'input has-error' : 'input'} id="jockeyImgUrl" name="imgUrl" type="url" placeholder="https://example.com/jockey.jpg" value={profileForm.imgUrl} onChange={handleProfileChange} disabled={isSavingProfile}/>
        ...
        <div className="admin-form-actions">
          ...
          {profile && (<button className="outline-button danger-action" type="button" onClick={handleDeactivateProfile} disabled={isSavingProfile}>
        ...
        <table className="user-table">
          ...
          <tbody>
            {items.map((invitation) => {
                ...
                return (<tr key={invitationId || `${invitation.tournamentId}-${invitation.horseId}`}>
                  ...
                  <td>
                    {isPending ? (<div className="table-actions">
                      ...
                      </div>) : (<span className="readonly-note">Đã xử lý</span>)}
                  </td>
                </tr>);
            })}
          </tbody>
    ...
    return (<AppShell variant="owner" title={`Xin chào, ${jockeyName}`} subtitle="Tạo hồ sơ jockey, theo dõi lời mời từ chủ ngựa và phản hồi lời mời thi đấu." profileName={jockeyName} profileRole={String(currentUser?.role || currentUser?.roleName || 'JOCKEY')} activeSection={activeSection} navItems={jockeyNavItems} onNavigate={handleNavigate} onLogout={onLogout} headerAction={<button className="refresh-button" type="button" onClick={reloadData} disabled={isLoading}>
          {isLoading ? 'Đang tải...' : 'Làm mới'}
        </button>}>
      {pageError && <div className="admin-alert error" role="alert">{pageError}</div>}
      {message && <div className="admin-alert success" role="status">{message}</div>}

      {activeSection === 'overview' && (<section className="owner-stack">
          ...
          <section className="owner-overview-grid">
            ...
            <div className="owner-panel compact-panel">
              ...
              <div className="owner-mini-list">
                {latestInvitations.length === 0 ? (<p className="table-empty">Chưa có lời mời nào.</p>) : latestInvitations.map((invitation) => (<div key={invitation.invitationId || `${invitation.tournamentId}-${invitation.horseId}`}>
      ...
      {activeSection === 'profile' && (<section className="owner-stack">
      ...
      {activeSection === 'invitations' && (<section className="owner-stack">
          {!profile && !isLoadingProfile && (<div className="admin-alert error" role="alert">Bạn chưa có profile jockey nên không thể accept invitation. Hãy tạo profile trước.</div>)}
          {profile && !isProfileActive && (<div className="admin-alert error" role="alert">Profile của bạn đang là {profile.status || 'N/A'}, không phải ACTIVE nên không thể accept invitation.</div>)}
