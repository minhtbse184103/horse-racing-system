...
export default function JockeyPendingDashboard({ currentUser, onLogout }) {
  ...
  const navItems = useMemo(() => [
    { key: 'overview', label: c.overview, icon: Gauge }, { key: 'professional-profile', label: c.professionalProfile, icon: UserRound, disabled: true, disabledReason: lockedReason }, { key: 'invitations', label: c.invitations, icon: List, disabled: true, disabledReason: lockedReason }, { key: 'application', label: c.application, icon: FileCheck2 }, { key: 'account', label: c.account, icon: UserRound }
  ], [c, lockedReason]);
    ...
    return (
      <section className="owner-stack">
        ...
        <section className="owner-overview-grid">
          <section className="owner-panel">
            <div className="owner-panel-header"><div><p className="eyebrow">{c.activationProgress}</p><h2>{c.unlockFeatures}</h2><p>{c.featuresAfterApproval}</p></div></div>
    ...
    if (activeSection === 'account') {
      return (
        <section className="owner-panel">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Detail label={c.username} value={currentUser?.username || currentUser?.fullName} /><Detail label={c.email} value={currentUser?.email} /><Detail label={c.phone} value={currentUser?.phone} /><Detail label={c.accountType} value="Jockey" /><Detail label={c.jockeyAccess} value={formatDisplayLabel(status)} />
          </div>
        </section>
      );
    }
