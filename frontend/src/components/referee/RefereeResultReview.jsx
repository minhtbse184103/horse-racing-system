...
function StatusBadge({ status }) {
  ...
  const className = normalized === 'SUBMITTED'
    ? 'border-amber-200 bg-amber-50 text-amber-800'
    : normalized === 'REFEREE_CONFIRMED'
        ...
        <header className="flex items-start justify-between gap-4 border-b border-brown-700/10 p-6">
          <div>
            <p className="text-xs font-black uppercase tracking-wide text-brown-500">
              {isFlag ? 'Flag Result' : 'Confirm Result'}
            </p>
    ...
    <section className="grid gap-5">
      <div className="rounded-lg border border-brown-700/10 bg-white/85 shadow-[0_16px_44px_rgba(43,23,16,0.08)]">
        ...
        <div className="grid gap-4 p-5 lg:grid-cols-4">
          ...
          <div className="rounded-lg bg-cream-100 p-4">
            <span className="text-xs font-black uppercase tracking-wide text-slate-500">Submission Time</span>
      ...
      {submission.reviewActions.length > 0 && (
        <section className="rounded-lg border border-brown-700/10 bg-white/80 p-5">
          <h3 className="text-lg font-black text-brown-900">Review Actions</h3>
...
export default function RefereeResultReview() {
  ...
  async function loadPendingSubmissions() {
    ...
    try {
      ...
      setListError(getErrorText(error, 'Không thể tải danh sách kết quả chờ duyệt.'));
    } finally {
  ...
  async function submitReview() {
    ...
    if (dialogMode === 'flag' && !reason) {
      setReviewError('Vui lòng nhập lý do flag result.');
      return;
    }
    ...
    try {
      if (dialogMode === 'flag') {
        ...
        returnToList('Result flagged.');
      } else {
        await confirmRaceResultSubmission(detail.submissionId, reason);
        returnToList('Result confirmed.');
      }
    } catch (error) {
      setReviewError(getErrorText(error, 'Không thể cập nhật trạng thái review.'));
    } finally {
  ...
  return (
    <section className="grid gap-5">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <MetricCard icon={Trophy} label="Pending submissions" value={submissions.length} note="SUBMITTED only" />
        <MetricCard icon={Clock3} label="Latest submission" value={submissions[0] ? formatDateTime(submissions[0].submittedAt) : 'N/A'} note="Submission time" />
        <MetricCard icon={AlertTriangle} label="Needs action" value={submissions.length} note="Confirm or flag" />
      </section>
      ...
      <section className="overflow-hidden rounded-lg border border-brown-700/10 bg-white/85 shadow-[0_16px_44px_rgba(43,23,16,0.08)]">
        <header className="flex flex-col gap-4 border-b border-brown-700/10 bg-cream-100/70 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-wide text-brown-500">Referee Result Review</p>
            <h2 className="mt-1 text-2xl font-black text-brown-900">Pending List</h2>
            ...
            <table className="min-w-[68rem] w-full text-left">
              <thead>
                <tr className="border-b border-brown-700/10 text-xs font-black uppercase tracking-wide text-brown-700">
                  ...
                  <th className="px-5 py-3">Race Date</th>
              ...
              <tbody>
                {filteredSubmissions.map((submission) => (
                  <tr className="border-b border-brown-700/10 last:border-b-0 hover:bg-cream-100/60" key={submission.submissionId}>
                    ...
                    <td className="px-5 py-4 font-bold text-brown-900">{formatDateTime(submission.raceStartTime)}</td>
