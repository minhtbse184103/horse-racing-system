      ...
      <div className="hidden xl:block">
        <table className="w-full table-fixed text-sm">
          ...
          <thead className="bg-cream-200/55 text-left text-[11px] font-black uppercase text-brown-700">
            <tr><th className="px-3 py-3">{t('eventDomainRegistration')}</th><th className="px-3 py-3">Horse / Jockey</th><th className="px-3 py-3">{t('eventDomainOwner')}</th><th className="px-3 py-3">Payment Status</th><th className="px-3 py-3">Approval Status</th><th className="px-3 py-3 text-right">{t('eventCommonViewDetail')}</th></tr>
          </thead>
          <tbody>
            {registrations.map((registration) => (
              <motion.tr key={registration.id} whileHover={{ x: 2 }} className="border-t border-brown-700/10 transition-colors hover:bg-cream-200/45">
                ...
                <td className="min-w-0 px-3 py-3.5"><p className="truncate font-black text-brown-900">{registration.horse}</p><p className="mt-0.5 truncate text-xs font-semibold text-slate-500">{registration.jockey || t('eventRegistrationNoJockey')}</p></td>
