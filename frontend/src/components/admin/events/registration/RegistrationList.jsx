    ...
    return (
      <div className="grid min-h-44 place-items-center p-6 text-center">
        <div>
          ...
          <p className="mt-3 font-black text-brown-900">Không tìm thấy Registration</p>
          <p className="mt-1 text-xs font-semibold text-slate-500">Điều chỉnh nội dung tìm kiếm hoặc bộ lọc Status để xem thêm kết quả.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="hidden xl:block">
        <table className="w-full table-fixed text-sm">
          ...
          <thead className="bg-cream-200/55 text-left text-[11px] font-black uppercase text-brown-700">
            <tr><th className="px-3 py-3">Registration</th><th className="px-3 py-3">Ngựa / Jockey</th><th className="px-3 py-3">Owner</th><th className="px-3 py-3">Payment Status</th><th className="px-3 py-3">Status duyệt</th><th className="px-3 py-3 text-right">Chi tiết</th></tr>
          </thead>
          <tbody>
            {registrations.map((registration) => (
              <motion.tr key={registration.id} whileHover={{ x: 2 }} className="border-t border-brown-700/10 transition-colors hover:bg-cream-200/45">
                ...
                <td className="min-w-0 px-3 py-3.5"><p className="truncate font-black text-brown-900">{registration.horse}</p><p className="mt-0.5 truncate text-xs font-semibold text-slate-500">{registration.jockey || 'Chưa phân công Jockey'}</p></td>
