    ...
    <div className="grid gap-2.5 border-b border-brown-700/10 bg-cream-200/30 p-3.5 lg:grid-cols-[minmax(15rem,1fr)_11rem_11rem_auto] lg:items-center">
      <label className="relative min-w-0">
        ...
        <input
          ...
          placeholder="Tìm Registration, ngựa hoặc Owner"
        />
      </label>
      <select value={filters.paymentStatus} onChange={(event) => onChange('paymentStatus', event.target.value)} className={FILTER_CLASS} aria-label="Lọc theo Payment Status">
        <option value={REGISTRATION_FILTER_ALL}>Tất cả Payment Status</option>
      ...
      <select value={filters.approvalStatus} onChange={(event) => onChange('approvalStatus', event.target.value)} className={FILTER_CLASS} aria-label="Lọc theo Status duyệt">
        <option value={REGISTRATION_FILTER_ALL}>Tất cả Status duyệt</option>
        {Object.entries(APPROVAL_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
      </select>
      <div className="flex items-center justify-between gap-3 lg:justify-end">
        <span className="inline-flex items-center gap-1.5 whitespace-nowrap text-xs font-extrabold text-slate-500"><SlidersHorizontal size={14} />{resultCount} kết quả</span>
        {hasFilters && (
