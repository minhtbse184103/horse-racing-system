...
export default function RegistrationReviewDialog({ registration, onClose, onDecision }) {
  ...
  async function decide(status, rejectionReason = null) {
    ...
    try {
      ...
      setError(decisionError.message || 'Unable to update this registration.');
    } finally {
    ...
    <motion.div {...modalBackdrop} className="fixed inset-0 z-[70] grid place-items-center bg-brown-900/65 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <motion.div {...modalPanel} className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-white/60 bg-cream-100 shadow-[0_32px_90px_rgba(43,23,16,0.46)]" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="registration-review-title">
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-brown-700/10 bg-white/75 p-5 md:p-6">
          <div className="min-w-0"><p className="truncate text-xs font-black uppercase text-brown-500">{registration.registrationNo}</p><h3 id="registration-review-title" className="mt-1 text-2xl font-black text-brown-900">Review registration</h3><p className="mt-1 text-sm font-semibold text-slate-500">Review the participant and audit record before making a decision.</p></div>
          <button type="button" onClick={onClose} className="grid size-9 shrink-0 place-items-center rounded-lg border border-brown-700/10 bg-white text-brown-700 hover:bg-cream-200" aria-label="Close"><X size={17} /></button>
        </header>
        ...
        <div className="min-h-0 overflow-y-auto p-5 md:p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <DetailItem label="Jockey" value={registration.jockey || 'Not assigned'} />
            <DetailItem label="Submitted at" value={formatOperationDateTime(registration.submittedAt)} />
            <DetailItem label="Reviewed at" value={formatOperationDateTime(registration.reviewedAt)} />
            <DetailItem label="Reviewed by" value={registration.reviewedBy || 'Not reviewed'} />
          </div>
          ...
          {registration.rejectionReason && <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4"><p className="text-xs font-black uppercase text-red-700">Rejection reason</p><p className="mt-2 text-sm font-bold text-red-900">{registration.rejectionReason}</p></div>}
          {error && <div className="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-900">{error}</div>}
          {isPending && (
            <label className="mt-5 grid gap-1.5 text-sm font-extrabold text-brown-900">Rejection reason
              <textarea value={reason} onChange={(event) => { setReason(event.target.value); setError(''); }} className="min-h-24 rounded-lg border border-brown-700/15 bg-white px-3.5 py-3 text-sm font-bold outline-none focus:border-brown-500 focus:ring-4 focus:ring-gold-400/15" placeholder="Required only when rejecting" />
            </label>
          )}
        </div>

        <footer className="flex shrink-0 flex-wrap justify-end gap-2 border-t border-brown-700/10 bg-white/70 p-4 md:px-5">
          <button type="button" disabled={isSubmitting} onClick={onClose} className="rounded-lg border border-brown-700/15 bg-white px-4 py-2.5 text-sm font-extrabold text-brown-700 hover:bg-cream-200 disabled:opacity-50">Close</button>
          {isPending && <><button type="button" disabled={isSubmitting} onClick={reject} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-extrabold text-danger hover:bg-red-100 disabled:opacity-50"><XCircle size={16} /> Reject</button><button type="button" disabled={isSubmitting} onClick={() => decide('APPROVED')} className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-extrabold text-white hover:bg-emerald-800 disabled:opacity-50">{isSubmitting ? <LoaderCircle className="animate-spin" size={16} /> : <CheckCircle2 size={16} />} Approve</button></>}
        </footer>
      </motion.div>
    </motion.div>
  );
