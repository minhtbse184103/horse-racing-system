...
export function createInitialTournamentDraft(initialTournament) {
  return {
    ...
    races: (initialTournament?.races || []).map(({ date, ...race }) => ({
      ...
      entryFinalizationScheduledAt: race.entryFinalizationScheduledAt || '',
      entryFinalizedAt: race.entryFinalizedAt || null,
      entryFinalizedBy: race.entryFinalizedBy || null,
