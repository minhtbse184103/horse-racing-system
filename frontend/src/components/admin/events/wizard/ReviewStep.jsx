        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)]">
          <div className="space-y-5">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <WizardSummaryItem icon={CircleDollarSign} label={t('eventWizardRacePrizeTotal')} value={`VND ${prizeTotal.toLocaleString()}`} />
              ...
              <div className="mt-4 divide-y divide-brown-700/10">
                {draft.races.map((race, index) => (
                  <div key={race.id} className="grid gap-3 py-3 first:pt-0 last:pb-0 sm:grid-cols-[2rem_minmax(0,1fr)_auto] sm:items-center">
                    ...
                    <div className="text-left sm:text-right"><p className="text-sm font-black text-brown-900">VND {race.prizes.reduce((sum, prize) => sum + Number(prize.amount || 0), 0).toLocaleString()}</p><p className="text-xs font-semibold text-slate-500">{t('eventWizardPrizeRanksConfigured', { count: race.prizes.length })}</p></div>
                  </div>
                ))}
