...
export default function ReviewStep({ draft, prizeTotal }) {
  const readiness = [
    { label: 'Thông tin Tournament đã hoàn tất', ready: Boolean(draft.name && draft.venue && draft.start && draft.end) },
    { label: `Đã cấu hình ${draft.conditions.length} điều kiện tham gia`, ready: draft.conditions.length > 0, optional: true },
    { label: `Đã cấu hình ${draft.races.length} Race`, ready: draft.races.length > 0 },
    { label: 'Mỗi Race đều có giải thưởng theo hạng', ready: draft.races.length > 0 && draft.races.every((race) => race.prizes.length > 0) }
  ];

  return (
    <div>
      <WizardSectionHeading eyebrow="Bước 4 trên 4" title="Xem lại và xác nhận" description="Kiểm tra toàn bộ Tournament, chương trình Race, điều kiện tham gia và tổng giải thưởng trước khi tạo." />
      <div className="mt-4">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)]">
          <div className="space-y-5">
            ...
            <section className="rounded-lg border border-brown-700/10 bg-white/80 p-5">
              <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase text-brown-500">Chương trình Race</p><h4 className="mt-1 text-lg font-black text-brown-900">{draft.races.length} Race đã cấu hình</h4></div><Flag className="text-brown-500" size={21} /></div>
            ...
            <section className="rounded-lg border border-brown-700/10 bg-white/80 p-5">
              <p className="text-xs font-black uppercase text-brown-500">Điều kiện tham gia</p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {draft.conditions.length === 0 && <span className="text-sm font-semibold text-slate-500">Không có giới hạn điều kiện tham gia.</span>}
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-5">
            <section className="rounded-lg border border-brown-700/10 bg-cream-200/50 p-5">
              <p className="text-xs font-black uppercase text-brown-500">Tóm tắt sự kiện</p>
              <dl className="mt-4 grid gap-3 text-sm">
                <div className="flex justify-between gap-4"><dt className="font-semibold text-slate-500">Mở Registration</dt><dd className="text-right font-black text-brown-900">{draft.registrationOpen || 'Chưa xác định'}</dd></div>
                <div className="flex justify-between gap-4"><dt className="font-semibold text-slate-500">Đóng Registration</dt><dd className="text-right font-black text-brown-900">{draft.registrationClose || 'Chưa xác định'}</dd></div>
                <div className="flex justify-between gap-4"><dt className="font-semibold text-slate-500">Sức chứa</dt><dd className="font-black text-brown-900">{draft.maxRegistration}</dd></div>
                <div className="flex justify-between gap-4"><dt className="font-semibold text-slate-500">Phí tham gia</dt><dd className="font-black text-brown-900">{formatVndCurrency(draft.entryFee)}</dd></div>
              </dl>
            </section>

            <section className="rounded-lg border border-brown-700/10 bg-white/80 p-5">
              <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-emerald-100 text-emerald-800"><ClipboardCheck size={17} /></span><div><p className="font-black text-brown-900">Kiểm tra sẵn sàng</p><p className="text-xs font-semibold text-slate-500">Tóm tắt kiểm tra dữ liệu frontend</p></div></div>
              <div className="mt-4 grid gap-3">
                {readiness.map((item) => (
                  <div key={item.label} className="flex items-start gap-2.5">
                    ...
                    <span className="text-xs font-bold leading-5 text-brown-900">{item.label}{item.optional && !item.ready ? ' (không bắt buộc)' : ''}</span>
                  </div>
                ))}
              </div>
            </section>
