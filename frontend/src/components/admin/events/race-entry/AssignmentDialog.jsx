    ...
    <motion.div {...modalBackdrop} className="fixed inset-0 z-[70] grid place-items-center bg-brown-900/65 p-4 backdrop-blur-sm" onMouseDown={isSubmitting ? undefined : onClose}>
      <motion.div {...modalPanel} className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg border border-white/60 bg-cream-100 shadow-[0_32px_90px_rgba(43,23,16,0.46)]" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="race-assignment-title">
        ...
        <footer className="flex shrink-0 flex-wrap items-center justify-end gap-3 border-t border-brown-700/10 bg-white/75 p-4 md:px-5">
          <p className="mr-auto text-xs font-semibold text-slate-500">{selectedRegistrationId == null ? 'Chọn một Registration để tiếp tục.' : 'Backend sẽ phân công ngẫu nhiên một vị trí xuất phát còn trống.'}</p>
          ...
          <button type="button" disabled={selectedRegistrationId == null || isFull || queueLoading || isSubmitting} onClick={confirmAssignment} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-brown-700 px-4 text-sm font-extrabold text-white hover:bg-brown-900 disabled:cursor-not-allowed disabled:bg-stone-300">{isSubmitting ? <LoaderCircle className="animate-spin" size={16} /> : <Check size={16} />} Phân công RaceEntry</button>
        </footer>
      </motion.div>
    </motion.div>
  );
