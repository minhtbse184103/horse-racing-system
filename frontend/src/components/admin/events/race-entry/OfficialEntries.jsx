  ...
  return (
    <div className="rounded-lg border border-brown-700/10 bg-white/75 p-3.5">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div><p className="text-xs font-black uppercase text-brown-500">{t('eventWorkspaceOfficialEntries')}</p><p className="mt-1 text-xs font-semibold text-slate-500">{t('eventRaceEntryOfficialEntriesSubtitle')}</p></div>
        <span className="rounded-full bg-cream-200 px-3 py-1.5 text-xs font-extrabold text-brown-700">{t('eventRaceEntryParticipants', { count: entries.length, max: race.maxRunners })}</span>
      </div>
      ...
      <div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-3">
        {[...entries].sort((a, b) => a.startingStall - b.startingStall).map((entry) => {
          return (
            <motion.article key={entry.id} whileHover={hoverLift} className="rounded-lg border border-white/80 bg-cream-100 p-3.5 shadow-[0_8px_22px_rgba(78,44,25,0.08)] hover:border-gold-400/40 hover:shadow-[0_14px_32px_rgba(78,44,25,0.13)]">
              ...
              <h4 className="mt-3 truncate font-black text-brown-900">{entry.horse || t('eventRaceEntryUnknownHorse')}</h4>
              <p className="mt-1 truncate text-xs font-semibold text-slate-500">{entry.registrationNo} · {entry.owner}</p>
              <p className="mt-1 truncate text-xs font-semibold text-slate-500">{t('eventDomainJockey')}: {entry.jockey || t('eventRegistrationNoJockey')}</p>
              <div className="mt-3 border-t border-brown-700/10 pt-3 text-xs font-semibold text-slate-500"><p>{formatOperationDateTime(entry.assignedAt)}</p><p className="mt-1">{t('eventRaceEntryAssignedBy')}: {entry.assignedBy}</p></div>
              {entry.cancelledAt && (
                <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-xs font-semibold text-red-900"><p>{t('eventStatus_CANCELLED')} {formatOperationDateTime(entry.cancelledAt)}</p><p className="mt-1">{t('eventRaceEntryCancelledBy', { name: entry.cancelledByName || entry.cancelledBy || t('eventRaceEntryUnknownAdmin') })}</p><p className="mt-1 font-bold">{entry.cancellationReason || t('eventRaceEntryNoReason')}</p></div>
              )}
              {entry.status === 'ASSIGNED' && canCancel && (
                <motion.button whileTap={tapPress} type="button" onClick={() => onCancel(entry)} className="mt-3 inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 text-xs font-extrabold text-red-700 hover:bg-red-100"><UserMinus size={14} /> {t('eventRaceEntryCancelTitle')}</motion.button>
              )}
        ...
        {entries.length === 0 && (
          <div className="col-span-full grid min-h-36 place-items-center rounded-lg border border-dashed border-brown-700/20 bg-cream-200/25 p-6 text-center">
            <div><CircleGauge className="mx-auto text-brown-500" size={22} /><p className="mt-3 text-sm font-black text-brown-900">{t('eventRaceEntryEmptyOfficialTitle')}</p><p className="mt-1 text-xs font-semibold text-slate-500">{t('eventRaceEntryEmptyOfficialHint')}</p></div>
          </div>
        )}
      </div>
    </div>
