  ...
  return (
    <div className={`flex ${compact ? 'gap-2' : 'justify-end gap-1.5'}`}>
      <motion.button
        ...
        title={canModify ? t('eventCommonEdit') : disabledReason}
        aria-label={canModify ? t('eventCommonEdit') : disabledReason}
      >
        <Pencil size={15} />
        {compact && t('eventCommonEdit')}
      </motion.button>
      <motion.button
        whileHover={{ y: -1 }}
        whileTap={tapPress}
      ...
      <motion.button
        ...
        title={canModify ? t('eventCommonCancel') : disabledReason}
        aria-label={canModify ? t('eventCommonCancel') : disabledReason}
      >
        <Trash2 size={15} />
        {compact && t('eventCommonCancel')}
      </motion.button>
    </div>
  );
}
